...
import { loadRegister } from './load-register.mjs';
...
export const DEFAULTS = Object.freeze({
  ...
  codeRevision: null,
  publicBaseUrl: null,
  rateLimitScope: 'per client IP, per process, fixed minute window',
...
// ponytail: this process-local throttle covers a single Node instance. Use
// the platform rate-limit binding for a fleet, as the Worker entry point does.
...
// documented upgrade path in deploy/README.md.
//
...
export function createApp(options = {}) {
  ...
  const throttle = options.throttle ?? createThrottle(config.rateLimitPerMinute, options.now);
  ...
  const listing = [{
    ...
    module: config.moduleUrn,
    label: 'Coal LSL payroll levy on eligible wages (s 3B)',
    description: 'Calculate eligible wages and levy for one eligible employee and one supported reporting month. The caller establishes eligibility and pay classification. Does not determine coverage, prepare a return or lodge it.',
    selection: { kind: 'fact' },
    method: 'eligible_wages_times_prescribed_percentage',
    supported_periods: [...periodUrns],
    input_schema_ref: '/openapi.json#/components/schemas/CoalLslLevyInput',
    invocation: { method: 'POST', path: `/v1/calculators/${config.calculatorUrn}/{period_uri}` },
    jurisdiction: 'AU',
    limits: {
      max_body_bytes: config.maxBodyBytes,
      requests_per_minute_per_client: config.rateLimitPerMinute,
      rate_limit_scope: config.rateLimitScope,
  ...
  const modules = [{
    module: config.moduleUrn,
    description: 'Australian coal industry long service leave payroll levy.',
    calculators: [config.calculatorUrn],
  }];
  ...
  async function route(request, response, cors, requestId) {
    ...
    const throttled = await throttle.take(clientKey(request, config), request);
    ...
    if (request.method === 'GET' || request.method === 'HEAD') {
      ...
      if (path === '/v1/modules') return send(response, 200, modules, headers);
      if (path === '/v1/calculators') {
        const module = new URL(request.url, 'http://localhost').searchParams.get('module');
        return send(response, 200, module === null || module === config.moduleUrn ? listingText : [], headers);
      }
