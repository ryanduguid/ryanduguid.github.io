...
// read on 18 September 2026 (snapshot: docs/contract-snapshot.md).
//
...
async function run(base) {
  ...
  for (const fixture of fixtures.cases) {
    ...
    const { response, body } = await post(periodUrn, fixture.request);
    ...
    if (response.status === 200) {
      ...
      if (JSON.stringify(repeat.body) !== JSON.stringify(body)) {
  ...
  check('5 fixture', `every fixture returns its expected status (${fixtures.cases.length} cases)`,
