...
test('discovery lists the calculator, its periods, limits and refusal classes', async () => {
  ...
  assert.deepEqual(entry.calc_uri_aliases, ['urn:sbrm:calc:coal-lsl-levy', 'urn:sbrm:calculator:coal-lsl-levy']);
...
test('modules, filtering and schema discovery describe a usable first call', async () => {
  const modules = await (await fetch(`${base}/v1/modules`)).json();
  assert.deepEqual(modules[0].calculators, [CALC]);
  const entries = await (await fetch(`${base}/v1/calculators?module=${modules[0].module}`)).json();
  assert.equal(entries.length, 1);
  assert.equal(entries[0].module, modules[0].module);
  assert.equal(entries[0].selection.kind, 'fact');
  assert.deepEqual(await (await fetch(`${base}/v1/calculators?module=unknown`)).json(), []);
  const [schemaUrl, pointer] = entries[0].input_schema_ref.split('#');
  let schema = await (await fetch(new URL(schemaUrl, base))).json();
  for (const key of pointer.split('/').slice(1)) schema = schema[key];
  assert.deepEqual(schema.required, ['reporting_month', 'branch', 'employee', 'pay']);
  const route = entries[0].invocation.path.replace('{period_uri}', PERIOD);
  assert.equal((await post(route, fixtures.cases[0].request)).status, 200);
});
